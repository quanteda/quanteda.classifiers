...
#' 
#' set.seed(1000)
#' tmod <- textmodel_cnnlstmemb(dfmat, y = docvars(dfmat, "crowd_immigration_label"),
#'                         epochs = 5, verbose = 1)
#' pred <- predict(tmod, newdata = dfm_subset(dfmat, is.na(crowd_immigration_label)))
#' table(pred)
#' tail(texts(corpuncoded)[pred == "Immigration"], 10)
#' }
textmodel_cnnlstmemb <- function(x, y, units = 512, dropout = .2, 
                            optimizer = "adam",
...
#' @export
textmodel_cnnlstmemb.dfm <- function(x, y, units = 512, dropout = .2, 
                                optimizer = "adam",
                                loss = "categorical_crossentropy", 
                                metrics = "categorical_accuracy", ...) {
